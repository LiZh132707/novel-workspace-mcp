...
命令行支持 `novel-workspace backup --list --novel NAME --json` 和 `novel-workspace backup --verify FILENAME --json`，用 `--output-dir` 指定备份目录。校验包含 CRC、路径、JSON 状态及 SHA-256 报告，上限为 10,000 个条目、1 GB 展开体积和 2 MB 状态文件。通过校验不代表内容完整、来源可信或已经验证过完整恢复流程。

### NPC 与人物关系

在 **人物管理 → 人物关系图谱** 中查看有向关系、强度（-100 至 100）和可展开的章节证据。支持焦点人物、角色层级（含 NPC）及截至章节筛选；NPC 视图同时显示关联人物，并按当前档案的出场范围筛选。

MCP 的 `get_character_network(chapter=10, character="小明", role_tier="NPC")` 使用同一查询逻辑，各筛选条件可省略，组合时取交集。历史查询不包含未标注章节的档案描述；层级和状态仍取当前档案，并非完整历史快照。观察记录不替代作者审核。`朋友：小明，阿青：导师` 等简单旧格式继续兼容，其余描述保留为备注而非虚构节点。
