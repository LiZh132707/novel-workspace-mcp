...
class BackupScheduler:
    def __init__(self, novels_root: Path, storage_root: Path, logger, interval_seconds: int = 3600, keep_per_novel: int = 7):
        self.novels_root = novels_root
        self.output = storage_root / "backups"
        self.logger = logger
    ...
    def _loop(self):
        while not self._stop.is_set():
            try:
                ...
                self.logger.exception("自动备份检查失败")
    ...
    def backup_due(self) -> list[Path]:
        ...
        for novel_path in self.novels_root.iterdir() if self.novels_root.exists() else []:
            if not novel_path.is_dir() or not (novel_path / "state.json").exists():
                continue
            backups = sorted(self.output.glob(f"{novel_path.name}_*.zip"), reverse=True)
    ...
    def create(self, novel_path: Path) -> Path:
            ...
            with FileLock(str(novel_path / ".novel_mutation.lock"), timeout=600):
                with zipfile.ZipFile(temporary, "w", zipfile.ZIP_DEFLATED) as archive:
                    for file in novel_path.rglob("*"):
                        if (
                            file.is_file()
                            and "exports" not in file.parts
                            and not file.name.endswith((".lock", ".tmp"))
                        ):
                            archive.write(file, file.relative_to(novel_path))
        ...
        self.logger.info("自动备份完成: %s", target.name)
        return target
