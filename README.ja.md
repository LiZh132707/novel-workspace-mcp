...
複数の伏線を選択し、項目ごとの差分を確認してから、目標章の移動・設定、優先度・状態の変更、タグの追加・削除を一括適用できます。全件のリビジョンを検証し、競合が一件でもあれば全体を中止します。作者/要約管理フィルターとクイック表示も追加しました。

適用済みの条件に一致する全ページの記録を JSON / Markdown に出力できます。メモと根拠は明示指定のみで、出力上限と完全性を表示します。CLI `novel-workspace foreshadows list|export|batch`、二つの MCP ツール、Skill ガイドも対応。モデル呼び出しや本文の書き換えはありません。[手順と制限](docs/FORESHADOW_BATCH.md) を参照してください。
