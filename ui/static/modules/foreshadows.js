...
(() => {
  ...
  function editValues(form,existing=false) {
    const data={text:form.text, target_chapter:Number(form.target_chapter),priority:form.priority,
      tags:form.tags.split(',').map(s=>s.trim()).filter(Boolean),notes:form.notes};
    if(existing) {
      data.status=form.status;
      if(data.status==='resolved') {
        if(form.resolved_chapter!=='') data.resolved_chapter=Number(form.resolved_chapter);
        data.resolution_note=form.resolution_note;
      }
    } else data.introduced_chapter=Number(form.introduced_chapter);
    return data;
  }
  if(typeof module!=='undefined') module.exports={renderBoard,boardURL,editValues};
